...
void StreamServer::applyPendingResize() {
    ...
    if (w <= 0 || h <= 0) return;
    if (w == capture_.getWidth() && h == capture_.getHeight()) return;

    std::unique_lock<std::mutex> pipelineLock(pipelineMutex_, std::try_to_lock);
    if (!pipelineLock.owns_lock()) return; // encode in progress, retry next frame
...
// 从候选 H.264 编解码器中挑选与编础器输出最匹配的一项。
// 评分依据：packetization-mode=1（+100 分，RTP 分片模式与 NVENC 输出格式匹配）
//           profile-level-id 前缀 42e0（约束基线 +50 分）> 42c0 > 4200 > 4d00 > 6400
